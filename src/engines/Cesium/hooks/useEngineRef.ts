...
import { CursorType, GeoidServer } from "../types";
import {
...
export default function useEngineRef(
  ...
  const geoidServer = useRef<GeoidServer | undefined>(undefined);
  ...
  const e = useMemo((): EngineRef => {
    return {
      ...
      setGeoidServer: (geoidServerParams: GeoidServer) => {
        geoidServer.current = geoidServerParams;
      },
      getGeoidHeight: async (lng?: number, lat?: number): Promise<number | undefined> => {
        if (!geoidServer.current?.url || !geoidServer.current?.geoidProperty) {
        ...
        return fetch(
          geoidServer.current.url
            .replace("${lng}", lng.toString())
            .replace("${lat}", lat.toString()),
        )
          .then(res => {
            return res.json().then((result: any) => {
              if (!result || !geoidServer.current?.geoidProperty) return undefined;
              const geoid = Number(result[geoidServer.current?.geoidProperty]);
              if (!isNaN(geoid)) {
